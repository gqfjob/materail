!function($){
  ...
  * ================================= */
  ...
  , select: function () {
	  ...
	  var all = $("#invite-btn").attr('data-src');
	  var tmps = this.trimValue(realVal, all);
	  if(tmps != all){
	      $("#inviteUser").append("<span style=\"margin-left:20px;\" id=\"userInfo-"+realVal+"\">"+val+"<a href=\"javascript:void(0);\" data-src=\""+realVal+"\" class=\"delUser\">×</a></span>");
	      $("#invite-btn").attr('data-src',tmps);
	  }//alert(realVal);
      return this.hide();
    }
  ...
  , trimValue:function (v, arr){ //查看数组里是否包含这个值,包含返回原串，否则用逗号组合起来返回
	  if(arr==''){
		 return v; 
	  }else{
		var r = arr.split(",");
