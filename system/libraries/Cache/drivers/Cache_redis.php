...
class CI_Cache_redis extends CI_Driver
{
     ...
     * @return  mixed
     */
    public function get($key,$region='')
    {
    	if(''!=$region)$this->_redis->setOption(Redis::OPT_PREFIX, $region.':');
     ...
     * @return  bool
     */
    public function save($key, $value, $ttl = NULL,$region='')
    {
    	if(''!=$region)$this->_redis->setOption(Redis::OPT_PREFIX, $region.':');
        return ($ttl)
     ...
     * @return  bool
     */
    public function delete($key,$region='')
    {
    	if(''!=$region)$this->_redis->setOption(Redis::OPT_PREFIX, $region.':');
     ...
     * @return  array
     */
    public function get_metadata($key,$region='')
    {
        $value = $this->get($key,$region);
        if(''!=$region)$this->_redis->setOption(Redis::OPT_PREFIX, $region.':');
        if ($value)
        {
            return array(
