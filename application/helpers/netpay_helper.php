...
class quickpay_conf
{
    ...
    static $pay_params  = array(
        ...
        'merId'         => '898320182990290', //商户填写
        'acqCode'       => '',  //收单机构填写
        'merCode'       => '',  //收单机构填写
        'merAbbr'       => '任意门-赶考网',//教育平台
    );
...
class quickpay_service
{
    ...
    const RESP_FAILED	= "01";	//交易异常，支付失败。详情请咨询95516
    ...
    {
        //<button type="submit" class="btn btn-primary"  style="display:none">去银联支付</button>
        $html = <<<eot
    <form id="pay_form" pay_type="1" name="pay_form" action="{$this->api_url}" method="post">
...
function netpay_query($orderNumber = '',$orderTime = ''){
	try{
        ...
        if ($queryResult == quickpay_service::QUERY_SUCCESS || $queryResult == quickpay_service::QUERY_FAIL) {
        	return $arr_ret;
        }else{
        	return FALSE;
        }
