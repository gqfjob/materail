...
class OAuth
{
    ...
    {
    	$CI =& get_instance();
    	$t_access_token = $CI->session->userdata('t_access_token');
    	$t_expire_in = $CI->session->userdata('t_expire_in');
    	$t_code = $CI->session->userdata('t_code');
    	$t_openid = $CI->session->userdata('t_openid');
    	$t_openkey = $CI->session->userdata('t_openkey');
    	$t_oauth_version = $CI->session->userdata('t_oauth_version');
        if (isset($t_access_token)) $CI->session->unset_userdata('t_access_token');
        if (isset($t_expire_in)) $CI->session->unset_userdata('t_expire_in');
        if (isset($t_code)) $CI->session->unset_userdata('t_code');
        if (isset($t_openid)) $CI->session->unset_userdata('t_openid');
        if (isset($t_openkey)) $CI->session->unset_userdata('t_openkey');
        if (isset($t_oauth_version)) $CI->session->unset_userdata('t_oauth_version');
    }
}
